...
$INPUT_COMPARTILHADO		=	<<<HTML
				<input type="checkbox" id="report_share" name="report_share" value="1"> <label for="report_share">Compartilhado</label>
HTML;
										
					  					
//Não tem permissãoi para GD				  					
if(!WRS_USER::getArrPerfUser(array('ADM','MST')))
{
	$DG_SELECT					=' ';	
	$INPUT_COMPARTILHADO		=	'';
}
...
$HTML	=	<<<HTML
				...
				<form id="insert_report" class="modal_report" name="insert_report">
								...
								<label for="report_name">Nome</label><br>
									<input class="change_title_report" type="text" id="report_name" name="report_name" value="{$nome_report}" style="width: 100%;">	
								<br>
								<input type="checkbox" id="report_auto" name="report_auto" value="1"> <label for="report_auto">Carga Automática </label>  &nbsp; &nbsp; &nbsp; 
								{$INPUT_COMPARTILHADO}	
							{$DG_SELECT}						
				</form>
