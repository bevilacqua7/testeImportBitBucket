...
class ATT_WRS_DATABASE extends WRS_BASE
{
	...
	public function __construct(){
		...
		$this->admin->classname = 'ATT_WRS_DATABASE';
		$this->queryClass		 = new QUERY_WRS_DATABASE();
	}
	...
	public function export($options=null)
	{
		...
		$param['title']= LNG('bt_export_database');
		return $param;
	}
	...
	public function update($options)
	{
		...
		$query_exec = $this->queryClass->Get_query_changetable_database($_tabela, $arr_campos_valores, ($acao_form=='UPDATE'?implode(' and ',$primaries):''), $acao_form);
	...
	public function delete($options)
	{
		...
		foreach($_regForDelete['objetosSelecionados'] as $id_for_del){
			...
			if(is_array($id_for_del)){
				...
				$id_for_del = $saida['DATABASE_ID'].','.$saida['SERVER_ID'];
			}
			
			$condicao_query = $id_for_del.",".$CUTOMER_ID.",".$USER_CODE;
			$query_exec = $this->queryClass->Get_procedure_remove_database($_tabela, $condicao_query);
	...
	public function import($options)
	{
		...
		$param['title']= LNG('bt_import_database');
