...
class WRS_PANEL  extends WRS_USER
{
	...
	{
		$_cube			=	 $this->getCube();
		$SERVER			=	$_cube['DATABASE_SERVER'];
		$USER			=	$_cube['DATABASE_USER'];
		$PWD			=	$_cube['DATABASE_PWD'];
		$DATABASE		=	$_cube['DATABASE_ID'];
		$CUBE			=	$_cube['CUBE_ID'];
		$gridFlag		=	$this->getGrid($SERVER,$USER,$PWD,$DATABASE,$CUBE);
	}
	 ...
	 * @param unknown $value
	 */	
	private function SAVE_CACHE_SSAS_USER($value)
	{
		WRS_TRACE('SAVE_CACHE_SSAS_USER()', __LINE__, __FILE__);	
		
		WRS::SET_SSAS_USER($this->_cube_pos_session,'TABLE_CACHE',$value);
	...
	protected function getMeasuteAttibute($SERVER, $USER, $PWD, $DATABASE, $CUBE,$CUBE_USER_CURRENT=NULL)
	{
		WRS_TRACE('getMeasuteAttibute', __LINE__, __FILE__);
		...
		//Verificando se existe informações do relatino ships na session
		$relationships			=	WRS::GET_SSAS_RELATIONSHIPS_BY_CUBE($CUBE);
		
		$cube_update			=	 $attibuteMeasure->check_CUBE_SSAS_UPDATE($SERVER, $USER, $PWD, $DATABASE, $CUBE);
		...
		//caso ainda não tenha sido inseria na sessão faz a pesquisa
		if(empty($relationships))
		{	
			$wrs_measure_relationships['relationships']	=	WRS::SET_SSAS_RELATIONSHIPS_BY_CUBE($CUBE, $attibuteMeasure->getAtributos($SERVER, $USER, $PWD, $DATABASE, $CUBE));
		}
		...
		//if(empty($metricas))
		{
				$wrs_measure_relationships['measure']	=	WRS::SET_SSAS_MEASURES_BY_CUBE($CUBE, $attibuteMeasure->getMetricas($SERVER, $USER, $PWD, $DATABASE, $CUBE));
		}
		...
		WRS_TRACE('END getMeasuteAttibute', __LINE__, __FILE__);
	 ...
	 * @return boolean
	 */
	public function getGrid($SERVER,$USER,$PWD,$DATABASE,$CUBE)
	{
...
//		$CUBE	=	'[SAN - MDTR_NEW][SAN - MDTR_NEW]';
		...
		if(count($get_multiple_SSAS_CUBE)>1)
		{
			...
			{
				$MEASURE_RELATIONSSHIPS[$multiple_cube['CUBE_ID']]	=	$this->getMeasuteAttibute($SERVER, $USER, $PWD, $DATABASE, $multiple_cube['CUBE_ID'],$CUBE);
			}
			
			$this->MEASURE_RELATIONSSHIPS		=	$MEASURE_RELATIONSSHIPS;
		}else{
			//Cubo simples 
			$this->getMeasuteAttibute($SERVER, $USER, $PWD, $DATABASE, $CUBE);
		}
		...
		{
				$queryGrid			=	 $this->_query->GET_SSAS_TABLE($SERVER, $USER, $PWD, $DATABASE, $CUBE, $ROWSL, $COLUMNS, $MEASURES, $FILTERS, $ALL_ROWS, $ALL_COLS, $COLS_ORDER);
				...
				if($this->num_rows($queryGrid_exec))
				{
					...
					$this->SAVE_CACHE_SSAS_USER($rows['TABLE_NAME']);
					$cube		=	 $this->getCube();
					
					/*
					 * Se existir mensagem de erro na coluna para o processo
					 */
					if(!empty($rows['MSG_ERROR']))
					{
						echo fwrs_warning(LNG('MSG_ERROR_TABLE_CACHE'),$getRequestKendoUi_TAG).fwrs_javascript('CLOSE_LOAD_RELATORIO();');
						WRS_TRACE('MSG_ERROR::  Erro ao verificar tabela temporparia '.$queryGrid, __LINE__, __FILE__);
						WRS_DEBUG_QUERY('MSG_ERROR::  Erro ao verificar tabela temporparia '.$queryGrid);
						return false;
					}

					
					//validando se é maios que 65 mil e se o usuári ope MST ou ADM
					$num_rows		=	(int) $rows['ROWS'];
					
					if($num_rows>=65000)
					{
						//Caso o usuário não seja Master ou Administrativo não pode visualizar a GRID
						if($this->getUserPerfil()!='MST' || $this->getUserPerfil()!='ADM')
						{
							echo fwrs_warning(LNG('USUARIO_SEM_PRIVILEGIOS_LIMITE_ROWS'),$getRequestKendoUi_TAG);
							WRS_TRACE('WARNING::  Usuário sem privilêgio para visualizar mais de 65 mil linhas', __LINE__, __FILE__);
							WRS_DEBUG_QUERY('WARNING::  Usuário sem privilêgio para visualizar mais de 65 mil linhas');
							return false;
						}
					}
					
				}else{
					WRS_TRACE('ERROR::GET_SSAS_TABLE não retornou informações QUERY:::'.$queryGrid, __LINE__, __FILE__);
		...
		$this->query($this->_query->SORT_TABLE($cube['TABLE_CACHE'],$LAYOUT_ROWS_SIZE,''));
		...
		$queryHeader		=	 $this->_query->GET_HEADER($SERVER, $DATABASE, $CUBE, $TABLE_NAME_CACHE, '[*]', $this->getUserLanguage());
...
//		$TelerikUi->setToolbarExcel('excel', 'ExportarExcel', $urlToExport);
	...
	public function SELECT_CACHE_GRID()
	{
		...
		if(isset($sort[0]['field']))
		{
			...
			$this->query($this->_query->SORT_TABLE($TABLE_NAME,$LAYOUT_ROWS_SIZE,$field,strtoupper($sort[0]['dir'])));
		}
		...
		$sqlGrid			=	 $this->_query->SELECT($TABLE_NAME.'S', $ROW_NUMBER_START, $ROW_NUMBER_END);
