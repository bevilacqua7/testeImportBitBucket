...
class WRS_LOGIN extends WRS_BASE
{
	...
	public function setLogin($USER_CODE,$USER_PWD)
	{
		...
		WRS_TRACE('Executando a query QUERY_LOGIN::SET_LOGIN ', __LINE__, __FILE__);
		$query			=	 QUERY_LOGIN::SET_LOGIN($USER_CODE, $USER_PWD, $ADDRESS, $SESSION, $BROWSER, $SYSTEM);
		...
		if($query)
		{
			...
			{
				$login_id		=	NULL;
				WRS_TRACE('Obtendo o resultado da query  QUERY_LOGIN::SET_LOGIN  ', __LINE__, __FILE__);
				...
				$query_get_login	= QUERY_LOGIN::GET_LOGIN($login_id);
