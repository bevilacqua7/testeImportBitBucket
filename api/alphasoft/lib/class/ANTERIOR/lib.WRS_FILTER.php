...
class WRS_FILTER  extends WRS_BASE
{
	...
	private function filter_select_info($cube_id,$cube_info)
	{
		...
		$RELATIONSHIP_KEY	=	fwrs_request('RELATIONSHIP_KEY'); 
		...
		*/
		...
		//Conexão
		$SERVER			=	$cube_info['DATABASE_SERVER'];
		$USER			=	$cube_info['DATABASE_USER'];
		$PWD			=	$cube_info['DATABASE_PWD'];
		...
		$query	=	$this->_query->FIND_FILTER_SELECT($SERVER, $USER, $PWD, $DATABASE, $CUBE, $ATTRIBUTE, $START, $END, $FILTER_UP, $LIKE, $MEASURE);
		...
		while($rows =  $this->fetch_array($query))
		{
			...
			$table_body.= $this->table_body($body,NULL,NULL,NULL,'pws_click_triger_single ' );
		}
...
HTML_ENTITIES;
	...
	public function getFilters($FILTER_ID)
	{
		...
		$query 		=	$this->_query->FILTER_RELATORIO($FILTER_ID);
		$query		=	$this->query($query);
		$filters_tmp=	array();
		$filters	=	NULL;
	...
	{		
		...
		$query			=	 $this->_query->RELATORIO_AUTO_LOAD($DATABASE_ID, $CUBE_ID);
