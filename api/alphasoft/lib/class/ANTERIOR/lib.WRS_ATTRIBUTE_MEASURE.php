...
class WRS_ATTRIBUTE_MEASURE  extends WRS_BASE
{
	 ...
	 * @return string
	 */
	public function check_CUBE_SSAS_UPDATE($SERVER, $USER, $PWD, $DATABASE, $CUBE)
	{
		$query 		= $this->_query->CUBE_SSAS_UPDATE($SERVER, $USER, $PWD, $DATABASE, $CUBE);
	 ...
	 */
	public function getAtributos($SERVER, $USER, $PWD, $DATABASE, $CUBE)
	{
			WRS_TRACE('getAtributos()', __LINE__, __FILE__);
			
			$LANGUAGE	=	 WRS::USER_LANGUAGE();
		
			$query 		=	 $this->_query->GET_SSAS_RELATIONSHIPS($SERVER, $USER, $PWD, $DATABASE, $CUBE, 0, $LANGUAGE);
			$query		=	 $this->query($query);
			$attr_tmp	=	array();
	 ...
	 * @return array
	 */
	public function getMetricas($SERVER, $USER, $PWD, $DATABASE, $CUBE)
	{
		...
		$measure_tmp	=	array();
		$query 			=	$this->_query->GET_SSAS_MEASURES($SERVER, $USER, $PWD, $DATABASE, $CUBE, 0, $LANGUAGE);
		$query			=	$this->query($query);
		$first_measure	=	true;
		if($this->num_rows($query))
		{
